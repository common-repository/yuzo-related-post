...
class Yuzo_Activator{
     ...
     * @since   6.1.3   2020-01-02  Now there are 9 pre-set Yuzo
     * @since   6.2     2020-02-26  Validate that if you have less than 30 posts, only the tag relationship Yuzo is activated
     * @return  void
     */
    public static function register_default_post(){
        ...
        // validate if you have more than 30 posts to be able to activate all Yuzo, but only activate 1
        $total_posts = $wpdb->get_row("SELECT count(*) total_posts FROM {$prefix}posts WHERE post_type in ('post','page')");
        $active_yuzo_basics = 1;
        if( (int)$total_posts->total_posts < 30){
            $active_yuzo_basics = 0;
        }
