 ...
 * @since		6.2			2020-02-25		Variable is added that keeps all the IDs shown by several Yuzo.
 *
 * @package    	Yuzo
...
final class Yuzo_Related {
	...
	$number_of_slack = 2,
	/**
	 * Store the IDs of the posts that have been displayed on a page.
	 *
	 * This is done as long as the global setting options 'Avoid repeated posts' are active
	 * which means that in each Yuzo query X post id is no longer displayed.
	 * This shows unique posts on the entire page.
	 *
	 * @since	6.2		2020-02-19	Release
	 * @access 	public
	 */
	$posts_id_already_shown = [];
	 ...
	 * @since	6.2			2020-02-25	Excludes IDs already shown in other Yuzo in case the user requires it
	 * @access 	public
	 *
	...
	public function get_sql_list_post( $opt = [] ){
		...
		// Valid if you do not want IDs to be repeated in several Yuzo
		// then include it in the query to exclude them
		if(  ! empty( $this->settings->general_avoid_repeated_posts ) && $this->settings->general_avoid_repeated_posts ){
			if(  ! empty( $this->posts_id_already_shown )  ){
				$this->sqlbuilder->whereNotIn( 'p.ID',  $this->posts_id_already_shown );
			}
		}
		...
		$ids = (array)$wpdb->get_results( $sql_string, ARRAY_A );
											//↑ Send the results in the form of an array

		// If the option 'general_avoid_repeated_posts' is activated, then it accumulates the IDs
		// in a class variable to avoid these id in the next execution of a Yuzo
		if(  ! empty( $this->settings->general_avoid_repeated_posts ) && $this->settings->general_avoid_repeated_posts ){
			$this->avoid_duplication_of_post( $ids );
		}

		yuzo::instance()->logs->log( 'List sql: ' . $sql_string );
	 ...
	 *
	 * @since 	6.0			2019-04-27	Release
	 * @since	6.0.9.4		2019-07-27	The most viewed algorithm was not working, now it works
	 * @since	6.0.9.7		2019-08-01	- The special character and ASC order filter was removed in the title-based realization
	 ...
	 *
	 * @since 	6.0		2019-04-27	Release
	 * @since	6.1.3	2020-01-02	This function was added as public
	 * @return 	array
	 */
	...
	/**
	 * Acumula los Ids que han sido ya mostrados en una variable de clase
	 *
	 * @since	6.2		2020-02-25		Release
	 * @return  void
	 */
	private function avoid_duplication_of_post( $ids ){
		$ids = yuzo_formater_ids( $ids );
		$this->posts_id_already_shown = \array_unique( array_merge( $ids, $this->posts_id_already_shown ) );
	}
