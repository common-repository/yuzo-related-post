                    ...
                    if ( ! empty( $values['customize_options'] ) ) {
                        foreach( $values['customize_options'] as $key => $customizes ) {
                            if( ! empty( $customizes ) && ! isset( self::$inited[$key] ) ) {
                                ...
                                self::$inited[$key] = true;
                                self::$args_instanced[$idElement]['customize'] = PF_Customize_Options::instance( $key, $params );
                            }
                        }
                    }
                    // For Shortcode
...
         * Perform several validations to see if the file is:
     ...
     * @since   1.6.33  2020-02-19  KEY correction for custom fields within the validation
     *
     * @return  string|html
     */
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
        ...
        if( ! empty( $field_type ) ) {
            ...
            if( ! empty( $field['is_custom'] ) && $field['is_custom'] == TRUE ){
                ...
                if( isset(self::$args_instanced[$field['element_id']]) ){
                    $path_custom = ! empty ( self::$args_instanced[$field['element_id']][$where]->args['custom_fields_path'] ) ?
                                            self::$args_instanced[$field['element_id']][$where]->args['custom_fields_path'] : null;
