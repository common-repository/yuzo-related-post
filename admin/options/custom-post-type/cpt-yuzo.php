 ...
 * @since   6.1.3       2020-01-02      Now you can see in the Yuzo list that template uses
 */
/*
|--------------------------------------------------------------------------
 ...
 *
 * @since   6.2     2020-02-25      Doc update.
 * @param   integer $post_id        Post number
 * @return  string
 */
function yuzo_views_in_column_cpt($post_id = 0){
    global $yuzo_settings;
    $return  = '';
    $views   = $views_source = yuzo_get_views( $post_id );
    ...
    $return .= '<span class="y-icons-levels y-icon-level'. $level .'"  title="'. $views_source .' Views"></span> ';
    $return .= '<span class="y-colors-levels y-color-level'. $level.'" title="'. $views_source .' Views">'. $views .'</span>';
    $return .= '<div></div>';
    ...
    ?>
        <div <?php if( $is_gunterberg ): ?>style="display:none;"<?php endif; ?> class="misc-pub-section yzp-metabox-stats - <?php echo $class_post_value; ?>">
            <div class="misc-pub-section-view misc-pub-section-view-level<?php echo $level; ?>">
                <span class="y-icons-levels y-icon-level<?php echo $level; ?>" title="<?php echo $view_source; ?> Views"></span>
                <span class="y-colors-levels y-color-level<?php echo $level; ?>" title="<?php echo $view_source; ?> Views"><?php echo $views; ?></span> </div>
