...
class Yuzo_AdminCustom{
     ...
     * @since   6.2     2020-02-25  Now if the view counter is cut this can be seen in the TITLE attribute by hovering over
     *
     * @return  string
     */
    public function create_adminbar_menu_yuzo_views() {
        ...
        if( is_singular() && $only_post_type_allow  && ( yuzo::instance()->admin::$is_user_panel_setting || yuzo::instance()->admin->isUserAllow() ) ){
            ...
            $views   = $view_source = yuzo_get_views( $post->ID );
            ...
            if( ! empty( $views ) ){
                ...
                $_html .= '<span class="y-icons-levels y-icon-level'. $level . $class_fix_position_icon . '" title="'.$view_source.' Views"></span> ';  //$views
                $_html .= '<span class="y-colors-levels y-color-level'. $level. $class_fix_position_view . '" title="'.$view_source.' Views">'. $views .'</span>';
...
left: 5px;
    ...
    /**
     * Header of the CPT of Yuzo
     *
     * @since   6.2     2020-02-25      - Update doc.
     *                                  - Validates that if you install it for the first time, it will redirect you to the 'about' page
     * @return  string
     */
    public function yuzo_header_in_cpt(){
        ...
        <?php
            // Valid if this is the first time you log in to redirect it to the "About" page
            $visited_about = yuzo_get_option('yuzo-config','visit_about');
            if( empty( $visited_about ) || $visited_about != 1 ){
                echo '<script>window.location.href="'. admin_url('admin.php?page=yuzo') .'"</script>';
            }
        ?>
     ...
     * @since   6.2     2020-02-25  Facebook group button has been added
     * @return  string
     */
    public function button_donate_options(){
        ...
        echo '<a href="http://bit.ly/2HOZXn9" target="_blank" class="yuzo-botton-fb-group-cpt button page-title-action">FB Group</a>';
    }
    ...
    public function form_feedback_uninstall(){
        ...
        $_html .="<div class='yuzo-popup'>";
            ...
            $_html .="<h3>".__('Quick Feedback','yuzo')."</h3>";
            $_html .="</div>";
    ...
    /**
     * Filter to modify the text when saving data in the Setting.
     *
     * @since   6.2     2020-02-25  Release
     * @return  string
     */
    public function change_text_after_save(){
        return esc_html(__( 'Setting saved (CTR + S)', 'yuzo' ));
    }

    /**
     * When you activate the DEBUG mode, a message appears in the ADMIN BAR that you have it active, this to notify.
     *
     * @since   6.2     2020-02-25      Release
     * @return  void
     */
    public function debug_mode_adminbar(){
        global $wp_admin_bar;
        if(  ! empty( yuzo::instance()->settings->general_mode_debug )  ){
            $menu_id = 'admin_yuzo_debugmode';
            $style = '<style>#wp-admin-bar-admin_yuzo_debugmode > div {background: #168CE7;color: #fff;}</style>';
            $wp_admin_bar->add_menu(array('id' => $menu_id, 'title' => __( 'Yuzo Debug is activated', 'yuzo' ) . $style, 'href' => ''));
        }
    }

    /**
     * If you are inside the ABOUT page, a value is saved to validate it
     *
     * @since   6.2     2020-02-25      Release
     * @return  void
     */
    public function validate_if_visited_aboutpage(){
        // Get the current screen, and check whether we're viewing the Yuzo
        $screen = get_current_screen();
        if ( 'toplevel_page_yuzo' == $screen->base ) {
            yuzo_update_option('yuzo-config', 1, 'visit_about');
        }
    }

    /**
     * Body class that helps identify the type of version
     * and at the same time it helps to mold the CSS better for vada type of version
     *
     * @since   6.2.2   2020-02-26  Release
     * @param   array   $classes    It has all kinds of body
     * @return  array
     */
    public function admin_body_class( $classes ){
        return $classes . ' yuzo-class-version-free';
    }
}
}